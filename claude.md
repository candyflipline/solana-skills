...
- `codegen.rs` - Rust program skeleton generation from spec. The legacy ParsedSpec-direct `generate()` covers Anchor + Quasar (escape hatch via `QEDGEN_LEGACY_CODEGEN=1`); Pinocchio is MIR-only (no legacy arm). This module also hosts the Pinocchio scaffold emitters (`emit_pinocchio_program_lib`, `emit_pinocchio_effect_body`) and the per-target SPL CPI dispatch (`try_emit_cpi` / `emit_spl_{anchor,quasar,pinocchio}`) that `codegen_mir` calls. Default routes through `codegen_mir.rs` post v2.30; 9 of 10 sub-generators MIR-direct, `generate_guards` still delegated to legacy pending the v3.0 typed-`Stmt` lift.
- `codegen_mir.rs` - v2.30 default codegen path for **all three** targets (Anchor / Quasar / Pinocchio). Sub-generator-level port — each `emit_<X>` is independently MIR-direct or delegates to the legacy `codegen::generate_<X>` (currently just `generate_guards`). Pinocchio is fully MIR-native (no legacy delegation): `#![no_std]` lib + byte-discriminant dispatch, zeropod zero-copy state, `&AccountInfo` account structs with `.handler()` methods, checked effects, SPL Token CPIs (`call Token.transfer(...)` → `pinocchio_token::instructions::*`), and plain-struct events.
...
- Per [[feedback-cleanup-v3]] the legacy modules stay reachable behind env-var escape hatches (`QEDGEN_LEGACY_{LEAN,KANI,CODEGEN,PROPTEST}=1`) through the v2.30→v2.31 soak. The Lean + Kani legacy deletion is **not** a clean delete: the `mir_out_of_scope` dispatch (`main.rs`) still forces legacy for sBPF (`pragma sbpf` → Lean *and* Kani) and record-bearing specs (`type T { … }`, e.g. percolator → Lean), because `lean_gen_mir`/`kani_mir` don't yet lift pragmas into MIR (`is_sbpf` is a Phase-0 stub) or emit record `structure`/`instance` declarations. So v2.32 is a **migration** release: port sBPF (Lean+Kani) and records (Lean) onto MIR, prove byte-parity on the `examples/sbpf/*` + percolator snapshots, *then* delete `lean_gen.rs` + `kani.rs` (~11K LoC) and drop the env-var hatches — see `docs/prds/RELEASE-v2.32.md`. v3.0 deletes codegen + proptest legacy (~10K LoC) after porting `generate_guards` + the proptest body to MIR-direct.
...
8a. **`old(...)` preservation harnesses (v2.23+)** — for every bundled spec whose `property` body contains `old(...)` (`grep -rl '\bold(' examples --include='*.qedspec'`), regen and confirm `tests/proptest.rs` emits the binary signature (`fn <prop>(pre: &State, post: &State) -> bool`) and the per-handler harness captures `let pre = s.clone(); let mut post = s;` before the handler call. Pre-v2.23 this lowered to a structural tautology silently. Bundled coverage today: `examples/regressions/issue-8/pool.qedspec` is the canonical pre/post test corpus; `examples/rust/percolator/percolator.qedspec`'s `old(...)` is in `ensures` and goes through the transition-fn assume path, unchanged by v2.23.
