...
- `codegen_mir.rs` - The Rust-codegen path (sole path since v2.32 deleted the legacy `codegen::generate`) for **all three** targets (Anchor / Quasar / Pinocchio). Each `emit_<X>` is MIR-direct; the account-constraint / guard surface (`generate_guards`, framework helpers, Pinocchio scaffold) is read from `ParsedSpec` via the shared `codegen_shared.rs` helpers. Pinocchio is fully MIR-native: `#![no_std]` lib + byte-discriminant dispatch, zeropod zero-copy state, `&AccountInfo` account structs with `.handler()` methods, checked effects, SPL Token CPIs (`call Token.transfer(...)` → `pinocchio_token::instructions::*`), and plain-struct events.
