...
8b. **Supply-chain gate** — `cargo audit --deny warnings --ignore RUSTSEC-2024-0436 --ignore RUSTSEC-2024-0388` and `cargo deny check` must both exit 0. CI's `supply-chain` job runs both on every push and PR. Install once with `cargo install --locked cargo-audit cargo-deny`. New RustSec advisories on transitive deps are the actionable signal; the two ignored IDs are informational "unmaintained" tags on `paste 1.0.15` (pulled by ark-ff-macros / bytemuck_derive) and `derivative 2.2.0` (pulled by ark-*) — drop the ignores when upstream cuts a release that removes them. License allowlist + registry / git-source pin live in `deny.toml`.
