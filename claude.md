...
- `lean_gen_mir.rs` - The Lean 4 codegen path (sole path since v2.32 deleted the legacy `lean_gen.rs`). Consumes `mir::Mir` and dispatches by spec shape: single/multi-account, indexed records, multi-variant ADT, and sBPF (`mir.is_assembly` → `render_sbpf`, which reads `ParsedSpec` directly for assembly proofs). Gated by `tests/mir_snapshot.rs` + the in-module `sbpf_render_matches_golden` test.
...
- Per [[feedback-cleanup-v3]] the Lean + Kani legacy was a **migration**, not a clean delete: `lean_gen_mir`/`kani_mir` first had to handle the shapes legacy still owned — sBPF (`pragma sbpf`) and record-bearing specs (`type T { … }`, e.g. percolator). v2.32 closed that: records → MIR (Lean), sBPF → MIR (Lean; Kani/proptest skip sBPF entirely — assembly is verified via Lean + client-side tests), then **deleted `lean_gen.rs` + `kani.rs` (~11K LoC)** and dropped the `QEDGEN_LEGACY_{LEAN,KANI}` hatches. `lean_gen_mir` is now the sole Lean path and `kani_mir` the sole Kani path. The sidecar writer was extracted to `lean_sidecars.rs` so it outlived `lean_gen.rs`. `codegen.rs` + `proptest_gen.rs` legacy remain behind `QEDGEN_LEGACY_{CODEGEN,PROPTEST}=1` — v3.0 deletes those (~10K LoC) after porting `generate_guards` + the proptest body to MIR-direct.
