...
pub fn render(mir: &Mir, parsed: &ParsedSpec) -> String {
    ...
    // Phase 3d+ stub. Remaining sections: file-level features
    // (covers / liveness / environment) — these live OUTSIDE
    // `emit_kani_account_section` in legacy, in
    // `emit_file_level_features` (single-mode only).
    out.push_str(
        "// MIR-TODO(phase-3d): file-level features (covers / liveness / environment) \
...
mod tests {
    ...
    #[test]
    fn render_emits_phase_3d_todo_marker() {
        // Post Phase 3c5+3c6+3c7 only file-level features (covers /
        // liveness / environment) remain. Every rendered file
        // carries the phase-3d TODO marker so users know what's
        // missing.
        let (mir, parsed) = lower_fixture("examples/rust/escrow/escrow.qedspec");
        let out = render(&mir, &parsed);
        assert!(
            out.contains("MIR-TODO(phase-3d)"),
            "expected phase-3d TODO marker"
        );
    }
